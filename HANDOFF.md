...
| `lessons.html` | PIN + 본인 선택 | 📖 공과공부 — 주별 공과 제목·유튜브 강의 링크(빨간 "▶️ 강의보기" 버튼)·상세 내용·첨부파일. Home(dashboard.html)에는 **이번 주 것만**(강의보기 버튼 포함) 미리보기 카드로 노출, 이 페이지에서 "📋 전체 공과 목록 보기"를 누르면 전체 17주(9~12월) 목록 확인 가능(강의 있는 주는 ▶️ 표시), 목록 클릭 시 상세로 전환. 상세 내용·영상링크 입력·수정·파일 첨부는 **관리자만**, 열람은 전체 교사. `lessons` 테이블(주차=일요일 날짜 PK, `video_url` 컬럼) + Storage 버킷 `lesson-files`(20MB, 공개) |
