...
| `prayer.html` | PIN + 본인 선택 | 🙏 학생상황+기도제목 (3개 탭, 전부 주차별 그룹 표시) — 우리 셀 탭은 **"이번 주 체크리스트"**(반 학생 전원 타일, 한 명씩 클릭해 입력/수정) 방식. 모든 입력은 share 값과 무관하게 항상 Supabase 동기화되어 같은 반/대가족 교사끼리 실시간 공유됨 |
| `teachers.html` | URL `?key=` | 🗂 셀편성 — 학생 상세·연락처·교적부·옷사이즈 |
| `relations.html` | URL `?key=` | 🕸 관계도 (1·3학기 토글) ※ 이전 파일명 index.html |
| `attendance-overview.html` | PIN | 📊 전체 출석현황 (모든 선생님 접근) + **⚠️ 관리가 필요한 학생**(급감/지속부진 자동 감지 → 클릭 시 출석스트립+교적부+조치사항 패널) |
...
| `lessons.html` | PIN + 본인 선택 | 📖 공과공부 — 주별 공과 제목·영상 링크 2종(보라 "📚 선행학습" + 빨강 "▶️ 강의보기")·상세 내용·첨부파일. Home(dashboard.html)에는 **이번 주 것만**(영상 버튼 포함) 미리보기 카드로 노출, 이 페이지에서 "📋 전체 공과 목록 보기"를 누르면 전체 17주(9~12월) 목록 확인 가능(있는 영상별로 📚/▶️ 표시), 목록 클릭 시 상세로 전환. 상세 내용·영상링크 입력·수정·파일 첨부는 **관리자만**, 열람은 전체 교사. `lessons` 테이블(주차=일요일 날짜 PK, `preview_video_url`/`lecture_video_url` 컬럼) + Storage 버킷 `lesson-files`(20MB, 공개) |
...
├── make_family_lineup_pptx.py (대가족별 반 편성 1장 요약 PPT 생성기 · family-lineup.pptx/.pdf — 선생님 배포용 유인물)
├── make_room_layout_pptx.py (공과공부 반별 배치도 PPT 생성기)
├── make_schedule_pptx.py (고등부 전체 일정 1장 유인물 생성기 · schedule-handout.pptx/.pdf, schedule 테이블 갱신 시 SCHEDULE 배열도 함께 갱신 필요)
...
## 2026-09-06 (2) — prayer.html: localStorage 전용 저장 버그 수정 + 주차별·반별 보기 + 체크리스트 입력 UX

- **🐛 핵심 버그 수정**: 기존엔 "전체 공유" 체크 안 한 학생상황·기도는 **입력한 교사의 브라우저 localStorage에만** 저장되고 Supabase에 동기화되지 않았음 → 같은 반 교사2(예: 1반 이유성/고은솔)도 서로의 미공유 입력을 못 보고, 다른 기기·브라우저 초기화 시 유실되는 문제였음. **이제 모든 입력이 share 값과 무관하게 항상 `prayers` 테이블에 동기화**됨 (`PrayerDB.upsert` 무조건 호출, `fetchSharedPrayers()`가 `share=eq.true` 필터 없이 전체 조회). `share` 필드는 이제 "대시보드·admin.html 등 전체 공개 노출 여부"만 통제(대시보드 쿼리는 기존대로 `share=eq.true` 유지) — 반/대가족 안에서는 공유 여부와 무관하게 항상 보임
- **주차별·반별 보기**: 기존엔 "학생별 모아보기" 탭만 날짜별 타임라인이었음 → **우리 셀·우리 대가족 탭도 전부 주차별(📅 날짜) 그룹 헤더로 표시**되도록 통일. 우리 대가족 탭에는 **반 필터 칩**(전체/1반/2반 등, 대가족 소속 반 2개 이상일 때만 노출) 추가 — `FAMILY_CELL_FILTER` 상태
- **입력 UX 전면 개편 (한 명씩 돌아가며 입력)**: 기존 드롭다운(학생 선택)+텍스트박스 방식 폼을 **"📋 이번 주 체크리스트"**로 교체 — 내 반 학생 전원이 타일로 나열, 이미 이번 주 입력된 학생은 ✅+미리보기, 미입력은 ⭕. 학생 타일 클릭 → 그 자리에서 바로 인라인 입력(이미 입력된 경우 기존 내용 프리필되어 수정 모드) → 저장하면 체크 갱신, 옆 학생으로 바로 넘어갈 수 있음. 공유·긴급 토글도 타일 안에 포함. **저장 시 저자(author) 상관없이 같은 반 교사 누구나 기존 항목을 열어 수정 가능**(공동 담임 협업 목적, 삭제도 가능) — 반면 아래 카드 목록의 "✏️ 수정"/"🗑 삭제"/"🌸 전체공유" 버튼은 기존대로 **본인이 작성한 카드에만** 노출(`isMine` 판정)
- **카드 인라인 수정 신규**: 기존엔 삭제만 가능했고 수정 기능 자체가 없었음 → 카드에 "✏️ 수정" 버튼 추가, 클릭 시 카드 내부가 textarea로 바뀌고 저장하면 `PrayerDB.update(id, {text})`로 PATCH(부분 업데이트 — 전체 row upsert가 아니므로 다른 필드 안전)
- **⚠️ 발견한 연쇄 버그**: 위 수정 중 `allPrayers()`가 Supabase 조회 결과 전체를 `fromGithub:true`로 태깅하던 로직이 있었음(원래는 정적 `data/prayers.json` 항목만 구분하려던 플래그였는데, PrayerDB.loadAll() 결과에도 잘못 재사용됨) → 로컬 캐시에 없고 Supabase에서만 불러온 "내가 쓴 글"이 수정 불가로 보이는 문제. `data/prayers.json` 항목에만 `_staticFile:true`를 태깅해 구분하도록 수정. **다른 브라우저/기기에서 로그인했을 때도 본인이 쓴 글의 수정·삭제·공유 버튼이 정상 노출되는지는 이 태깅 로직에 달려 있음 — 향후 회귀 주의**
- 브라우저 시뮬레이션으로 검증: 이유성(1반 교사1)이 오은수 기도제목 입력 → 고은솔(1반 교사2, localStorage 완전 초기화한 별도 세션)이 즉시 조회 가능 확인 → 인라인 수정 → Supabase에 실제 반영 확인(curl로 재조회)
...
- **attendance-overview.html: "⚠️ 관리가 필요한 학생" 섹션 신규** — 상단 통계 아래 항상 노출. `allWeeksRange()`(1학기 시작~오늘 전체 주차) 기준으로 각 학생의 **현재 연속 결석 스트릭**(가장 최근 주부터 역산, `○` 아닌 값 나오면 중단 — dashboard.html 내 반 카드 알림과 동일 기준) 계산 → 2주 이상이면 노출. 누적출석률 ≥50%면 **🔔 급감**(그동안 잘 나오다 최근 이탈), <50%면 **🤍 지속부진**으로 자동 분류. 학생 클릭 시 오른쪽 슬라이드 패널(students.html과 동일 패턴)에서 ① 최근 16주 출석 스트립 ② 🔒 교적부(연락처·가족, students.html과 동일 Vault·같은 코드) — **읽기 전용**(수정은 students.html에서만) ③ **조치사항·상황메모**(`student_notes` 테이블 재사용, `attend` 타입 신규 추가·기본 선택값) — 전체 교사 작성 가능, 삭제는 본인 글+국장단만. **이 페이지는 기존부터 전체 교사가 전체 학생 출석을 보는 페이지라, students.html의 반별 열람 제한과 무관하게 여기서는 국장단이 아니어도 모든 학생의 조치사항·교적부를 볼 수 있음** (의도적 — 이탈 관리는 담임 개인이 아니라 팀 전체가 인지해야 하는 사안이라 판단)
