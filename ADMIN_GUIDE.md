...
## 올리는 법
1. 대시보드 → **📸 학생 사진 관리** 클릭 (또는 `photos.html` 직접 접속)
2. PIN + 본인 이름으로 접속
3. 화면에 본인 반 학생 카드가 뜸 (관리자는 전체 학생)
4. 학생 카드의 **카메라 버튼(📷)** 클릭 → 사진 파일 선택
5. 자동으로 정사각형으로 잘려 압축·업로드됨 → **즉시 저장**
6. 다른 선생님 화면에는 **1~2분 내** 반영 (이름 옆 동그란 사진)

## 알아두기
- **권한**: 일반 선생님은 본인 셀 학생만, 관리자(국장단·목사·총무)는 전체 학생.
...
## 사진 삭제가 필요하면
업로드 화면엔 삭제 버튼이 없습니다(실수 방지). 삭제는 총무(석준원)에게 요청 → Supabase 대시보드에서 처리.
