...
>
> **변경 이력 (2026-06-06, 구현 후):**
> 1. 전용 페이지 → 대시보드 인라인 업로드로 변경했다가,
> 2. 다시 **전용 페이지 `photos.html` 로 복귀**(업로드는 거기서만). 추가로:
>    - 모든 교사 페이지(dashboard/attendance/attendance-overview/prayer/teachers) 이름 옆 아바타 + **클릭 시 라이트박스 확대**
>    - 모든 교사 페이지 상단 **공통 네비 바**(site-nav)로 상호 연결, teachers의 "출석현황 준비중" → 실제 연결
>    - attendance-overview 에도 아바타 추가
>    - 공개용 `assignments.html`·관계도 `index.html` 노드에는 사진 미표시(개인정보)
> 저장소·키·권한 모델은 아래 설계 그대로 유지.
